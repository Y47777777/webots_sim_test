...
AGVController::AGVController() : BaseLidarControl("webots_shadow_perception") {
    ...
    whileSpinPushBack(bind(&WBase::spin, lidar_0_ptr_));
    whileSpinPushBack(bind(&WBase::spin, lidar_3_ptr_));
...
void AGVController::Slam2ReportSpin() {
    LOG_INFO("%s start --> %s", __FUNCTION__, lidar_3_webots_topic.c_str());
    while (!webotsExited_) {
        sendPointCloud(lidar_3_webots_topic, lidar_3_ptr_,
                       pose_ptr_, lidar_3_webots_base_topic);
    }
}
