...
AGVController::AGVController() : BaseLidarControl("webots_shadow_perception") {
    ...
    // 创建线程
    m_thread_.insert(
        std::pair<std::string, std::thread>("lidar_3_report", std::bind(&AGVController::Slam1ReportSpin, this)));
    m_thread_.insert(
        std::pair<std::string, std::thread>("lidar_0_report", std::bind(&AGVController::Slam2ReportSpin, this)));
}
