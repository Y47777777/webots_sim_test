...
//         LOG_INFO("angleAxis %.2f, %.2f, %.2f, %.2f", angleAxis.angle(),
//                  angleAxis.axis().x(), angleAxis.axis().y(),
//                  angleAxis.axis().z());

//         // 将轴角转换为四元数
//         Eigen::Quaterniond q = Eigen::Quaterniond(angleAxis);
//         LOG_INFO("q  %.2f, %.2f, %.2f, %.2f", q.x(), q.y(), q.z(), q.w());

//         // 通过四元数创建旋转矩阵
//         Eigen::Matrix3d rotationMatrix = q.normalized().toRotationMatrix();
//         LOG_INFO("rotationMatrix ");
//         LOG_INFO("%.2f, %.2f, %.2f,", rotationMatrix(0, 0),
//                  rotationMatrix(0, 1), rotationMatrix(0, 2));
//         LOG_INFO("%.2f, % .2f, % .2f,", rotationMatrix(1, 0),
//                  rotationMatrix(1, 1), rotationMatrix(1, 2));
//         LOG_INFO("%.2f, % .2f, % .2f,", rotationMatrix(2, 0),
