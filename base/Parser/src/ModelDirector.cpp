...
void ActuatorDirector::create(const char *name,
  ...
  } else if (strcmp(name, "ForkDeviceZ") == 0) {
    ptr = std::make_shared<ForkDeviceZ>();
    // std::cout << "create ForkDeviceZ = " << name << std::endl;
  } else if (strcmp(name, "ForkDeviceP") == 0) {
    ptr = std::make_shared<ForkDeviceP>();
    // std::cout << "create ForkDeviceP = " << name << std::endl;
  ...
  if (isSupportModified) {
    support_handle_[name] = total_ - 1;
    // std::cout << "name = " << name  << ", handle = " << support_handle_[name]
    // << std::endl;
  }
  return;
}
...
int ActuatorDirector::get(const char *name) {
  return support_handle_[name];
}
...
void SencerDirector::create(const char *name,
                            const std::vector<struct SpecialParam> &param,
                            int length, bool is32bits) {
  ...
  if (strcmp(name, "WheelCoder") == 0) {
    ...
    // std::cout << "create IncrementalSteeringCoder = " << name << std::endl;
  } else if (strcmp(name, "Gyroscope") == 0) {
    ptr = std::make_shared<Gyroscope>();
    // std::cout << "create Gyroscope = " << name << std::endl;
  } else if (strcmp(name, "ForkDisplacementSencerZ") == 0) {
    ptr = std::make_shared<ForkDisplacementSencerZ>();
    // std::cout << "create ForkDisplacementSencerZ = " << name << std::endl;
  ...
  } else if (strcmp(name, "AccelerometerX") == 0) {
    ptr = std::make_shared<AccelerometerX>();
    // std::cout << "create AccelerometerX = " << name << std::endl;
  } else if (strcmp(name, "AccelerometerY") == 0) {
    ptr = std::make_shared<AccelerometerY>();
    // std::cout << "create AccelerometerY = " << name << std::endl;
  } else if (strcmp(name, "AccelerometerZ") == 0) {
    ptr = std::make_shared<AccelerometerZ>();
    // std::cout << "create AccelerometerZ = " << name << std::endl;
  } else if (strcmp(name, "AngularVelocitySensorX") == 0) {
    ptr = std::make_shared<AngularVelocitySensorX>();
    // std::cout << "create AngularVelocitySensorX = " << name << std::endl;
  } else if (strcmp(name, "AngularVelocitySensorY") == 0) {
    ptr = std::make_shared<AngularVelocitySensorY>();
    // std::cout << "create AngularVelocitySensorY = " << name << std::endl;
  } else if (strcmp(name, "AngularVelocitySensorZ") == 0) {
    ptr = std::make_shared<AngularVelocitySensorZ>();
    // std::cout << "create AngularVelocitySensorZ = " << name << std::endl;
  } else {
...
int SencerDirector::get(const char *name) {
  return support_handle_[name];
}
...
const struct Package *SencerDirector::get() {
  ...
  crc = CalCRC(package_.buf, 0, package_.len - tail_s_to_e - crc_s_to_e, 0xffff,
               0x8408);
