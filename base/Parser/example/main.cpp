...
int main(void) {
  ...
  // // decoder.solveValue(ForkDeviceZ_Handle, ForkDeviceZ, &real_ForkDeviceZ);
  ...
  // // encoder.updateValue("WheelCoder", WheelCoder_input, fake_direct_input);
  ...
  //             left WheelCoder_cal_input.push_back(mWheelCoder_wheel_r_code);
  ...
  std::string path_decode_st =
      ...
      "/workspace/vn_source/base/Parser/example/Actuators_ST.config";
  ...
  decoder_st.getValue("SwitchActuator", &switch_sensor, &nothing);
  decoder_st.getValue("SteeringDevice", &SteeringDevice, &realSteeringDevice);
  printf("[");
  for (int i = 0; i < 10; i++) { printf("%.2X ", switch_sensor[i]); }
  printf("]\n");
  printf("[");
  int counter = 0;
  int step = 0;
  ...
  std::string path_st_encode =
      std::string(root_path) +
      "/workspace/vn_source/base/Parser/example/Sencers_ST.config";
  ...
  encoder_st.updateValue("VelocityControlLevel", nullCalculatedInput,
                         VelocityControlLevelInput);
  encoder_st.updateValue("RPMSensor", RPMSensorInput, nullDirectInput);
  encoder_st.updateValue("SwitchSencer", nullCalculatedInput,
                         SwitchSencerInput);
  encoder_st.updateValue("AccelerometerX", AccelerometerXInput,
                         nullDirectInput);
  encoder_st.updateValue("AccelerometerY", AccelerometerYInput,
                         nullDirectInput);
  encoder_st.updateValue("AccelerometerZ", AccelerometerZInput,
                         nullDirectInput);
